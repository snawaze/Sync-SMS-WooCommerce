...
(function($) { 
"use strict"; 
    $("#syncsms_setting\\[syncsms_woocommerce_sms_from\\]").focusout(function () {
        var sender_id = $("#syncsms_setting\\[syncsms_woocommerce_sms_from\\]").val().trim();
        ...
        $("#syncsms_setting\\[syncsms_woocommerce_sms_from\\]").val(sender_id);
    });

    $("#admin_setting\\[syncsms_woocommerce_admin_sms_recipients\\]").focusout(function () {
        var admin_mobile_no = $("#admin_setting\\[syncsms_woocommerce_admin_sms_recipients\\]").val().trim();
    ...
    const setupPhoneHelper = function () {
        let selectedValue = $("#multivendor_setting\\[syncsms_multivendor_selected_plugin\\]").val();
        let phoneFieldLocation = '[edit profile page > SyncsmsAPI WooCommerce > phone]';
    ...
    $("#multivendor_setting\\[syncsms_multivendor_selected_plugin\\]").change(setupPhoneHelper);
    setupPhoneHelper();

    $('#syncsms_sms\\[open-keywords\\]').click(function (e) {
        ...
        if ($('#syncsms_new_billing_field') && $('#syncsms_new_billing_field').val() !== '') {
            let newFields = $('#syncsms_new_billing_field').val().split(',');
        ...
        const buildTable = function (keywords) {
            ...
            chunkedKeywords.forEach(function (row, rowIndex) {
                ...
                row.forEach(function (col) {
                    tableCode += `<td class="column"><button class="button-link" onclick="syncsms_bind_text_to_field('${target}', '[${col}]')">[${col}]</button></td>`;
                });
                tableCode += '</tr>';
        ...
        $('#syncsms_sms\\[keyword-modal\\]').off();
        $('#syncsms_sms\\[keyword-modal\\]').on($.modal.AFTER_CLOSE, function () {
        ...
        $('#syncsms_sms\\[keyword-modal\\]').html(mainTable);
        $('#syncsms_sms\\[keyword-modal\\]').modal();
    });
})(jQuery); 

function syncsms_bind_text_to_field(target, keyword) {
