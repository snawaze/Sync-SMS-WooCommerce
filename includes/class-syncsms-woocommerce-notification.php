...
class Syncsmssms_WooCommerce_Notification
{
    protected $log;
    ...
    {
        $this->log->add('Syncsms', 'Order status changed": old status: ' . $old_status . ' , new status: ' . $new_status);
    }

    public function woocommerce_payment_complete($order_id)
    {
        $this->log->add('Syncsms', 'Payment completed');
    }

    public function woocommerce_payment_complete_order_status($order_id)
    {
        $this->log->add('Syncsms', 'Completed order status');
    }

    public function send_customer_notification($order_id, $status)
    {
        if (! in_array($status, syncsms_get_options('syncsms_woocommerce_send_sms', 'customer_setting', array()))) {
            return;
        }
        ...
        if (Syncsmssms_Multivendor_Factory::$activatedPlugin == "wc_marketplace") {
            ...
            if ($is_suborder) {
                ...
                $send_sms_flag = false;
                if (syncsms_get_options('syncsms_woocommerce_suborders_send_sms', 'customer_setting', 'off') == 'on') {
                    $send_sms_flag = true;
                }
            }
        }
        //Checking if multivendor is "dokan"
        if (Syncsmssms_Multivendor_Factory::$activatedPlugin == "dokan") {
        ...
        if (Syncsmssms_Multivendor_Factory::$activatedPlugin == "yith") {
            //checking if it's a suborder
        ...
        if ($send_sms_flag) {
            $message = syncsms_get_options('syncsms_woocommerce_sms_template_' . $status, 'customer_setting', '');
            if (empty($message)) {
                $message = syncsms_get_options('syncsms_woocommerce_sms_template_default', 'customer_setting', '');
            }
            if (empty($message)) {
                return;
            }
            ...
            if ($customer_phone_no !== false) {
                $this->log->add('Syncsms', 'Customer\'s billing phone number (' . $order_details->get_billing_phone() . ') in country (' . $order_details->get_billing_country() . ') converted to ' . $customer_phone_no);
            } else {
    ...
    public function send_admin_notification($order_id, $status)
    {
        ...
        if (! in_array($status, syncsms_get_options('syncsms_woocommerce_admin_send_sms_on', 'admin_setting', array()))) {
            return;
        }
        ...
        if (Syncsmssms_Multivendor_Factory::$activatedPlugin == "wc_marketplace") {
            ...
            if ($is_suborder) {
                $send_sms_flag = false;
                if (syncsms_get_options('syncsms_woocommerce_admin_suborders_send_sms', 'admin_setting', 'off') == 'on') {
                    $send_sms_flag = true;
                }
            }
        }
        ...
        if ($send_sms_flag) {
            $message        = syncsms_get_options('syncsms_woocommerce_admin_sms_template', 'admin_setting', '');
            $message        = $this->replace_order_keyword($message, $order_details, 'admin', $status);
            $admin_phone = trim(syncsms_get_options('syncsms_woocommerce_admin_sms_recipients', 'admin_setting', ''));
            //Get default country v1.1.17
            $admin_country = syncsms_get_options('syncsms_woocommerce_country_code', 'syncsms_setting', '');
                ...
                foreach ($phone_no_array as $number) {
                    if ($number != '') {
                        ...
                        if ($phone_with_country_code !== false) {
                            $this->log->add('Syncsms', 'Admin\'s phone number (' . $number . ') in country (' . $admin_country . ') converted to ' . $phone_with_country_code);
                        } else {
    ...
    {
        require_once plugin_dir_path(__DIR__) . 'lib/syncsms.php';

        $api_key = syncsms_get_options("syncsms_woocommerce_api_key", 'syncsms_setting', '');
        $service = syncsms_get_options("syncsms_woocommerce_service", 'syncsms_setting', '');
        $whatsapp = syncsms_get_options("syncsms_woocommerce_whatsapp", 'syncsms_setting', '');
        $device = syncsms_get_options("syncsms_woocommerce_device", 'syncsms_setting', '');
        $gateway = syncsms_get_options("syncsms_woocommerce_gateway", 'syncsms_setting', '');
        $sim = syncsms_get_options("syncsms_woocommerce_sim", 'syncsms_setting', '');

        $this->log->add('Syncsms', 'Sending message to ' . $phone_no . ', message: ' . $message);

        try {
            $syncsms_rest = new Syncsms($api_key, $service, $whatsapp, $device, $gateway, $sim);
            $rest_response  = $syncsms_rest->sendSMS($phone_no, $message);

            $this->log->add('Syncsms', 'Response from gateway: ' . $rest_response);
        } catch (Exception $e) {
            $this->log->add('Syncsms', 'Failed sending message: ' . $e->getMessage());
        }
    }

    protected function check_and_get_phone_number($phone_number, $country)
    {
        $selected_country_code        = syncsms_get_options('syncsms_woocommerce_country_code', 'syncsms_setting', '');

        $check_phone_number_request_url = 'https://syncsms.net/woocommerce/mobile?phone={{phone}}&country={{country}}';
        ...
        if (is_array($response)) {
            ...
            $this->log->add('Syncsms', 'check number api err response:' . $customer_phone_no);

            return false;
        }

        $this->log->add('Syncsms', 'check number api timeout, continue send without formatting');
