...
class Syncsmssms_Multivendor_Notification extends Syncsmssms_WooCommerce_Notification
{
    /* @var Abstract_Syncsmssms_Multivendor $syncsms_multivendor */
    private $syncsms_multivendor;
    private $medium;
    ...
    public function __construct($medium = 'wordpress_woocommerce_multivendor', $syncsms_multivendor = null, Syncsmssms_WooCoommerce_Logger $log = null)
    {
        parent::__construct($log);
        if ($syncsms_multivendor === null) {
            $syncsms_multivendor = Syncsmssms_Multivendor_Factory::make();
        }
        $this->syncsms_multivendor = $syncsms_multivendor;
        $this->medium                = $medium;
    }

    public function send_to_vendors($order_id, $status)
    {
        if (syncsms_get_options('syncsms_multivendor_vendor_send_sms', 'multivendor_setting', 'off') === 'off') {
            return;
        }
        $send_sms_flag = true;
        ...
        if ($send_sms_flag) {
            ...
            $activatedHooks = syncsms_get_options('syncsms_multivendor_vendor_send_sms_on', 'multivendor_setting', $this->defaultHooks);
            if (! in_array($status, $activatedHooks)) {
                $this->log->add('Syncsms_Multivendor', 'not sending, current hook: ["' . $status . '"] activated hooks: ' . json_encode($activatedHooks));
                return;
            }

            $this->log->add('Syncsms_Multivendor', '3rd party plugin setting: ' . syncsms_get_options('syncsms_multivendor_selected_plugin', 'multivendor_setting', 'auto'));
            if (! $this->syncsms_multivendor) {
                $this->log->add('Syncsms_Multivendor', 'error: no multivendor plugin detected');
                return;
            }
            $this->log->add('Syncsms_Multivendor', 'activated plugin: ' . Syncsmssms_Multivendor_Factory::$activatedPlugin);

            $order_details = wc_get_order($order_id);
            $message       = syncsms_get_options('syncsms_multivendor_vendor_sms_template', 'multivendor_setting', '');
            //Get default country v1.1.17
            $default_country = syncsms_get_options('syncsms_woocommerce_country_code', 'syncsms_setting', '');

            $vendor_data_list = $this->syncsms_multivendor->get_vendor_data_list_from_order($order_id);
            if (! $vendor_data_list) {
                return;
            }
            foreach ($vendor_data_list as $phone_number => $vendor_datas) {
                $phone_number = $this->phone_number_processing($phone_number);
                $this->log->add('Syncsms', 'Original template: ' . $message);
                $processed_msg = $this->replace_vendor_order_keyword($message, $order_details, $vendor_datas);
                //Country Code v1.1.17
                $vendor_country = $this->syncsms_multivendor->get_vendor_country_from_vendor_data($vendor_datas);
                ...
                if ($phone_with_country_code !== false) {
                    $this->log->add('Syncsms', 'Vendor\'s phone number (' . $phone_number . ') in country (' . $vendor_country . ') converted to ' . $phone_with_country_code);
                } else {
    ...
    public function replace_vendor_order_keyword($message, WC_Order $order_details, $vendor_datas)
    {
        ...
        $replace = array(
            ...
            get_bloginfo('url'),
            $this->syncsms_multivendor->get_vendor_shop_name_from_vendor_data($vendor_datas),
        ...
        $this->log->add('Syncsms', "Total replaced keyword: $total_replaced");

        return $message;
    }

    // 2020-07-04 - Support additional billing field for Multivendor
    // Copied from class-syncsms-woocommerce-notification.php
